...
//Required
const authOptions = {
    authority,                                                          //The OIDC provider base domain
    authorizeEndpoint: `${authority}/connect/authorize`,                //The OIDC authorize endpoint (can be retrieved from /.well-known/openid-configuration endpoint on OIDC provider)
    endSessionEndpoint: `${authority}/connect/endsession`,              //The OIDC endsession endpoint (can be retrieved from /.well-known/openid-configuration endpoint on OIDC provider)

    redirectUri: `${currentDomain}/Callback`,                           //OAuth 2.0 client_redirect_uri (must be known to OIDC provider)
    silentRenewCallbackUri: `${currentDomain}/SilentRenew`,             //Must also be known/registered as a client redirect_uri with the OIDC provider
